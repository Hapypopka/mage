...
def sell_on_auction(page):
    ...
    while True:
        ...
        if comp_gold > 0 or comp_silver > 0:
            ...
            log("⚠️ Конкуренты не найдены — используем цену по умолчанию")
            new_gold = 0
            new_silver = 10  # Дефолтная цена
...
def cleanup_backpack_if_needed(page):
    ...
    # Открываем бонусы (Бонус подземелий и т.д.)
    open_bonus_items(page)

    # Разбираем всё (без аукциона)
